...
void D3D::SetRenderTarget(ID3D11RenderTargetView* rtv, ID3D11DepthStencilView* dsv)
{
	...
	gDC->OMSetRenderTargets(1, &rtv, dsv);
}
...
void D3D::SetGpuInfo(void)
{
	...
	//DisplayMode 중에서 선택한 width, height 와 같은 설정을 찾는다.
	for (UINT i = 0; i < modeCount; i++)
	{
		if (displayModeList[i].Width == d3dDesc.Width 
			&& displayModeList[i].Height == d3dDesc.Height)
		{
...
void D3D::CreateSwapChainAndDevice(void)
{
...
void D3D::CreateBackBuffer(float width, float height)
{
...

	{
		...
		hr = gDevice->CreateTexture2D(&desc, NULL, &backBuffer);
		assert(SUCCEEDED(hr));
	}

	{
		...
		hr = gDevice->CreateRenderTargetView(pBackbuffer, NULL, &renderTargetView);
		assert(SUCCEEDED(hr));
...

	{
		...
		hr = gDevice->CreateDepthStencilView(backBuffer, &desc, &depthStencilView);
		assert(SUCCEEDED(hr));
...
void D3D::DeleteBackBuffer(void)
{
